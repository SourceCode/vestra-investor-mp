...
import { Drawer, List, ListItemButton, ListItemAvatar, ListItemText, Typography, Avatar, Box, IconButton, Badge } from '@mui/material';
import { Close } from '@mui/icons-material';
import { useDispatch, useSelector } from 'react-redux';
import { RootState, setInboxDrawerOpen, setActiveConversation, fetchConversationsRequest } from '../store';
import MessageThread from './MessageThread';

const InboxDrawer: React.FC = () => {
    const dispatch = useDispatch();
    const { isInboxDrawerOpen } = useSelector((state: RootState) => state.ui);
    const { conversations, activeConversationId } = useSelector((state: RootState) => state.messaging);
    ...
    return (
        <Drawer
            ...
            PaperProps={{ sx: { width: { xs: '100%', md: 800 }, display: 'flex', flexDirection: 'row' } }}
        >
