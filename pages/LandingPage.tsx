...
import { Container, Typography, Button, TextField, InputAdornment, Box, Grid } from '@mui/material';
import { LocationOn, TrendingUp, Speed, Security, PieChart } from '@mui/icons-material';
        ...
        <Container maxWidth="lg">
          ...
          <Grid container spacing={3}>
            {MOCK_PROPERTIES.slice(0, 4).map((prop) => (
              <Grid item xs={12} sm={6} md={3} key={prop.id}>
      ...
      <section className="bg-slate-50 py-24" aria-labelledby="why-heading">
        <Container maxWidth="lg">
          ...
          <Grid container spacing={8}>
            {[
              { icon: <PieChart fontSize="large" aria-hidden="true" />, title: "Curated Deals", desc: "Every listing is pre-vetted for investment potential with clear value-add angles." },
              { icon: <Speed fontSize="large" aria-hidden="true" />, title: "Fast Underwriting", desc: "Instant access to ARV, rehab estimates, and projected ROI calculators." },
              { icon: <Security fontSize="large" aria-hidden="true" />, title: "Transparent Bids", desc: "Make offers with confidence using our standardized contract and closing process." }
            ].map((item, i) => (
