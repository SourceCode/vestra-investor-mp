    ...
    <Paper className="p-4 mb-3 rounded-xl border border-slate-200 hover:shadow-md transition-shadow cursor-pointer">
        <div className="flex justify-between items-start mb-2">
            <div className="flex items-center gap-2">
                <Avatar sx={{ width: 24, height: 24, fontSize: 12 }}>{offer.userName[0]}</Avatar>
        ...
        <div className="flex justify-end">
            <DealStatusChip status={offer.status === 'SUBMITTED' ? 'OFFER_SUBMITTED' : offer.status === 'ACCEPTED' ? 'OFFER_ACCEPTED' : 'CANCELLED'} size="small" />
        </div>
    </Paper>
);
