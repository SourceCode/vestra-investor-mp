import { ArrowBack, Build, CalendarToday, Chat, Dashboard, Favorite, FavoriteBorder, Lock, Share, VerifiedUser } from '@mui/icons-material';
import { Alert, Box, Button, Chip, Container, Divider, Grid, Paper, Tab, Tabs, Typography } from '@mui/material';
...
const MetricTile = ({ label, subtext, value }: { label: string, subtext?: string; value: string, }) => (
...
import { AppDispatch, fetchPropertyDetail, openConversationForDealRequest, RootState, setOfferDrawerOpen, toggleSaveRequest } from '../store';
...
const PropertyDetailPage: React.FC = () => {
    ...
    const { detail: reduxProperty, loading: reduxLoading } = useSelector((state: RootState) => state.properties);
    ...
    // tRPC Query
    const { data: realProperty, isLoading: realLoading } = trpc.marketplace.getListing.useQuery(
        { id: id! },
        { enabled: !!id && useRealData }
    );

    useEffect(() => {
        if (id && !useRealData) dispatch(fetchPropertyDetail(id));
    }, [id, dispatch, useRealData]);

    const property = useRealData ? realProperty : reduxProperty;
    const loading = useRealData ? realLoading : reduxLoading;
    ...
    const handleMessageAgent = () => {
        ...
        if (!property.assignedAgentId) {
            showToast('Agent information not available', 'error');
            return;
        }

        // Optimistically redirect or waiting for mutation?
        // Let's create the conversation
        createConversationMutation.mutate({
            dealId: property.id,
            participantIds: [user?.id!, property.assignedAgentId],
            title: `Inquiry: ${property.address}`
        });
    };
        ...
        <div className="bg-white min-h-screen pb-20">
            ...
            <Container maxWidth="lg" className="mt-8">
                <Grid container spacing={6}>
                    <Grid size={{ xs: 12, md: showTransaction ? 12 : 8 }}>
                        {/* Header */}
                        <div className="mb-8">
                        ...
                        <Box sx={{ borderBottom: 1, borderColor: 'divider' }} className="mb-6">
                            ...
                            >
                                ...
                                <Tab label="My Offers" id="tab-3" aria-controls="panel-3" />
                                {showTransaction && <Tab label="Transaction Workspace" id="tab-4" aria-controls="panel-4" />}
                            </Tabs>
                        </Box>
                        ...
                        <div role="tabpanel" hidden={tabValue !== 3} id="panel-3" aria-labelledby="tab-3">
                            {tabValue === 3 && <OfferList dealId={property.id} />}
                        </div>

                        <div role="tabpanel" hidden={tabValue !== 4} id="panel-4" aria-labelledby="tab-4">
                            {tabValue === 4 && showTransaction && <TransactionWorkspace property={property} />}
                        </div>
                    </Grid>
                </Grid>
