...
import { Dashboard, Favorite, FavoriteBorder, History, Logout, Mail, Menu as MenuIcon, Notifications, PersonOutline, Settings, WifiOff } from '@mui/icons-material';
import { Alert, AppBar, Avatar, Badge, Box, Button, Container, Divider, IconButton, ListItemIcon, Menu, MenuItem, Toolbar, Typography } from '@mui/material';
...
import { logout, RootState, setInboxDrawerOpen } from '../store';
...
const AppShell: React.FC<AppShellProps> = ({ children }) => {
  ...
  // Use tRPC for real-time notification count
  ...
  // Also get incomplete tasks count? (Mocked for now or added later)
  const notifUnread = unreadCount || 0;
        ...
        <Container maxWidth="xl">
          <Toolbar disableGutters className="h-20 justify-between gap-4">
            ...
            <nav aria-label="Primary" className="flex items-center gap-8 shrink-0">
              ...
              {/* Org Switcher */}
              {isAuthenticated && <div className="ml-4 border-l pl-4 border-slate-200 h-8 flex items-center"><OrgSwitcher /></div>}
              ...
              <div className="hidden lg:flex gap-6">
                ...
                <Button color="inherit" onClick={() => navigate('/admin/analytics')} className="font-medium text-slate-600">Analytics</Button>
              </div>
            </nav>
            ...
            <div className="flex items-center gap-2 md:gap-4 shrink-0">
              {isAuthenticated ? (
                <>
                  ...
                  >
                    <Avatar sx={{ bgcolor: tenant.primaryColor, fontSize: '1rem', height: 40, width: 40 }} alt={`${user?.firstName} ${user?.lastName}`}>
